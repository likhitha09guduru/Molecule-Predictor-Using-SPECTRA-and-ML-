...
class DataTransformation:
    ...
    loaders = transformer.create_dataloaders(processed_data)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Step 3: Transform to DataLoaders
        ...
        if dataloaders.get('train') is not None:
            ...
            if 'target' in batch:
                ...
                if transformer.target_type == 'regression':
                    print(f"  Target values (first 5): {batch['target'][:5].tolist()}")
                else:
                    print(f"  Target classes (first 5): {batch['target'][:5].tolist()}")
